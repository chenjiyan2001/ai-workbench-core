...
#### 📋 变更摘要
重构 Git 规范文档结构，补齐常见高风险操作场景，并统一 rebase/force-push 权限规则与提交规范写法。

#### ✨ 新增 (Added)
- **development/git-operations.md**
  - 新增操作指南，覆盖 8 个典型场景
  - 新增场景：`cherry-pick` 挑拣提交、`git restore` 撤销改动、`git reflog` 后悔药、`hotfix` 紧急修复流程
- **development/git-claude.md**
  - 新增 Claude 行为规范与权限矩阵，明确可执行/需确认/禁止的操作边界
- **development/git.md**
  - 新增分支模型说明
  - 新增推荐 Git 配置
  - 新增提交规范：`BREAKING CHANGE` 写法、Issue 关联格式（`Refs`/`Closes`）

#### 🐛 修复 (Fixed)
- 修复 `rebase` / `force-push` 规则冲突：改为条件性权限

#### 🔧 变更 (Changed)
- 将原 `git.md` 拆分为三份文档：
  - `git.md`（基础规范）
  - `git-operations.md`（操作指南）
  - `git-claude.md`（Claude 行为规范）
...
#### 📋 变更摘要
补充 Claude/Codex 协作分工规范，并强化 Git 文档的自然语言触发与高级场景决策指引。

#### ✨ 新增 (Added)
- **CLAUDE.md**
  - 新增 Claude/Codex 协作分工章节，明确职责边界与交付形态
- **development/git.md**
  - 新增 Claude/Codex 任务分工表，统一协作流程与角色定位
  - 新增自然语言触发识别规则（如用户提出"帮我开发功能"→建议采用 `feature/*` 分支策略等）
  - 新增 6 个高级 Git 场景决策树：撤销回滚、分支合并、暂存恢复、同步更新、查看对比、清理维护
...
#### 💬 备注
本次为规范文档增强，旨在提升协作一致性与复杂 Git 操作的决策效率。
